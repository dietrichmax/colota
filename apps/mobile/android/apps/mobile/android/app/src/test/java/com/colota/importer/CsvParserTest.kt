...
class CsvParserTest {
    ...
    fun `parses Colota CSV export header order`() {
        ...
        // CsvFormat header: id,timestamp,iso_time,latitude,longitude,accuracy,altitude,speed,battery
        val csv = """
    ...
    fun `accepts foreign column order via header lookup`() {
        // Foreign CSVs (Strava, custom spreadsheets) have the same fields in different
        // orders. The parser maps by header name, not by position, so this should work.
        val csv = """
            time,lat,lon,acc
    ...
    fun `falls back from numeric timestamp to ISO when numeric is blank`() {
        // Tolerates a partially populated row: numeric ts blank, ISO column present.
        val csv = """
    ...
    fun `rejects header missing required columns`() {
        // No lat/lon means we can't import anything; surface a clear error rather
    ...
    fun `drops malformed rows but continues parsing`() {
        // A single bad row shouldn't abort the whole import.
        val csv = """
    ...
    fun `skips blank lines without counting them as invalid`() {
        // Blank lines are common in hand-edited CSVs (trailing newline, separator
        // between sections). They're not data, not malformed data.
        val csv = """
            timestamp,latitude,longitude
            1704110400,52.5,13.4
    ...
    fun `parseHeader maps aliases to canonical indices`() {
        // Common aliases (lat/lng/velocity etc.) should map correctly so we accept
        // exports from other apps without forcing the user to rename columns.
