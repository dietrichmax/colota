...
Your server endpoint must accept HTTP or HTTPS requests (POST or GET depending on your HTTP method setting). The field's placeholder shows the shape the chosen backend template expects. The address is saved when you leave the field, not on every keystroke, and only when it passes: an address without `http://` or `https://` and a host is refused, and plain `http://` to a public host is refused with "http is refused for a public host. Use https." Plain `http://` to a private host (`192.168.x`, `10.x`, `172.16-31.x`, `100.64.x`, `localhost`) is saved with a warning that it is not encrypted, and an address carrying a key in its query string is saved with a warning that the key is stored with settings, not encrypted, and included in setup links.
